...
export async function handleApiRequest(request, response) {
  ...
  try {
    ...
    if (method === "GET" && url.pathname === "/api/health") {
      getAuthConfig();
      sendJson(response, 200, { status: "ok", auth: "configured" });
      return true;
    }
    ...
    console.error(`[Valax API] ${url.pathname} failed:`, error instanceof Error ? error.message : "Unknown error");

    if (url.pathname === "/api/callback") {
      redirect(response, "/login?error=oauth_failed");
    } else {
      sendJson(response, 500, { error: "The authentication service is temporarily unavailable." });
    }
    return true;
  }
}
