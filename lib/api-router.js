...
async function getServerContext(request, response, guildId) {
  if (!/^\d{17,20}$/.test(guildId || "")) {
    sendJson(response, 400, { code: "GUILD_ID_INVALID", error: "Select a valid Discord server." });
    return null;
  }
...
async function getServerWorkspace(request, response, url) {
  ...
  const context = await getServerContext(request, response, guildId);
  if (!context) return;
...
async function getServerMessages(request, response, url) {
  ...
  let messageResponse;
  try {
    messageResponse = await fetch(`${DISCORD_API}/channels/${channelId}/messages?limit=50`, {
      headers: discordBotHeaders(context.token),
      signal: AbortSignal.timeout(8_000),
    });
  } catch {
...
function allowedMentions(policy) {
  if (policy === "all") return { parse: ["users", "roles", "everyone"] };
  if (policy === "users") return { parse: ["users"] };
  return { parse: [] };
}
...
async function sendServerMessage(request, response) {
  ...
  const latestMessage = await context.database.collection("messageLogs").findOne(
    { userId: context.user._id, channelId },
    { sort: { createdAt: -1 } }
  );
  ...
  try {
    discordResponse = await fetch(`${DISCORD_API}/channels/${channelId}/messages`, {
      method: "POST",
      headers: { ...headers, "Content-Type": "application/json" },
      body: JSON.stringify({ content, allowed_mentions: allowedMentions(mentionPolicy) }),
      signal: AbortSignal.timeout(8_000),
    });
  } catch {
