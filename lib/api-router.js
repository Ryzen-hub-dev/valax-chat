...
const MAX_SAVED_EXPRESSIONS = 250;
...
function requestUrl(request) {
  ...
  const consolidatedPath = url.searchParams.get("__valaxPath");
  ...
  if (url.pathname === "/api/index" && apiPaths.has(consolidatedPath)) {
    url.pathname = `/api/${consolidatedPath}`;
    url.searchParams.delete("__valaxPath");
  }
  const consolidatedRoute = url.searchParams.get("__valaxRoute");
  const serverRoutes = new Set(["messages", "members", "expressions", "expression-library", "dm", "dm-recent", "dm-campaigns", "notifications"]);
  if (url.pathname === "/api/server" && serverRoutes.has(consolidatedRoute)) {
    url.pathname = `/api/server/${consolidatedRoute}`;
    url.searchParams.delete("__valaxRoute");
  }
  return url;
}
...
async function readBodyBuffer(request, maximumBytes) {
  if (Buffer.isBuffer(request.body)) {
    if (request.body.length > maximumBytes) throw new Error("Request body is too large.");
    return request.body;
  }
...
function sanitizeUploadName(value) {
  ...
  return leaf.replace(/[^a-zA-Z0-9._ ()-]/g, "_").slice(0, 120) || "attachment";
}

function validateUpload(file) {
  if (!file || typeof file.arrayBuffer !== "function" || file.size <= 0) return null;
  if (file.size > MAX_UPLOAD_BYTES) {
    const error = new Error("Attachments must be 4 MB or smaller.");
    error.code = "ATTACHMENT_TOO_LARGE";
    throw error;
  }
  const contentType = String(file.type || "").toLowerCase();
  if (!ALLOWED_UPLOAD_TYPES.has(contentType)) {
    const error = new Error("Choose a PNG, JPEG, WebP, AVIF, GIF, MP4, WebM, or MOV file.");
    error.code = "ATTACHMENT_TYPE_INVALID";
    throw error;
  }
  return { file, contentType, filename: sanitizeUploadName(file.name) };
}

async function readMessageRequest(request) {
  const contentType = String(request.headers["content-type"] || request.headers["Content-Type"] || "");
  if (!contentType.toLowerCase().startsWith("multipart/form-data")) {
    return { body: await readJsonBody(request), attachment: null };
  }

  const source = await readBodyBuffer(request, MAX_MULTIPART_BYTES);
  ...
  if (typeof payload !== "string" || payload.length > 16_384) throw new Error("Message metadata is invalid.");
  const body = JSON.parse(payload);
  const attachment = validateUpload(form.get("file"));
  return { body, attachment };
}

async function discordMessageBody(payload, attachment) {
  if (!attachment) {
    return {
  ...
  form.append("files[0]", new Blob([await attachment.file.arrayBuffer()], { type: attachment.contentType }), attachment.filename);
...
function stickerAssetUrl(id, formatType, size = 320) {
  if (formatType === 4) return `https://cdn.discordapp.com/stickers/${id}.gif?size=${size}&quality=lossless`;
  if ([1, 2].includes(formatType)) return `https://cdn.discordapp.com/stickers/${id}.png?size=${size}&quality=lossless`;
  return `https://media.discordapp.net/stickers/${id}.webp?size=${size}&quality=lossless`;
}
...
function publicDiscordMessage(message) {
  ...
  return {
    ...
    attachments: Array.isArray(message.attachments)
      ...
      : [],
    stickers: Array.isArray(message.sticker_items)
      ? message.sticker_items.map((sticker) => ({
          id: sticker.id,
          name: sticker.name || "Discord sticker",
          formatType: sticker.format_type || 1,
          url: stickerAssetUrl(sticker.id, sticker.format_type || 1),
        }))
      : [],
    embeds: Array.isArray(message.embeds)
      ? message.embeds.slice(0, 4).map((embed) => ({
          ...
          thumbnail: embed.thumbnail?.proxy_url || embed.thumbnail?.url || null,
          video: embed.video?.proxy_url || embed.video?.url || null,
        }))
      : [],
  };
}
...
async function getServerExpressions(request, response, url) {
  const guildId = url.searchParams.get("guildId")?.trim() || "";
  const context = await getServerContext(request, response, guildId, url.searchParams.get("botId") || "");
  if (!context) return;
  ...
  let stickerResponse;
  try {
    [emojiResponse, stickerResponse] = await Promise.all([
      fetch(`${DISCORD_API}/guilds/${guildId}/emojis`, {
        headers: discordBotHeaders(context.token),
        signal: AbortSignal.timeout(8_000),
      }),
      fetch(`${DISCORD_API}/guilds/${guildId}/stickers`, {
        headers: discordBotHeaders(context.token),
        signal: AbortSignal.timeout(8_000),
      }),
    ]);
  } catch {
    sendJson(response, 502, { code: "DISCORD_API_UNREACHABLE", error: "Valax could not reach Discord while loading expressions." });
    return;
  }

  if (emojiResponse.status === 401 || stickerResponse.status === 401) {
    sendJson(response, 401, { code: "BOT_TOKEN_INVALID", error: "Discord rejected the saved Bot Token." });
    return;
  }
  const unexpected = [emojiResponse, stickerResponse].find((result) => !result.ok && ![403, 404].includes(result.status));
  if (unexpected) {
    sendJson(response, 502, { code: "DISCORD_API_ERROR", error: "Discord could not return this server's expressions." });
    return;
  }

  const emojis = emojiResponse.ok ? await emojiResponse.json() : [];
  const stickers = stickerResponse.ok ? await stickerResponse.json() : [];
  sendJson(response, 200, {
    emojis: emojis
    ...
    stickers: stickers
      .filter((sticker) => sticker.available !== false)
      .map((sticker) => ({
        id: sticker.id,
        name: sticker.name || "Discord sticker",
        description: sticker.description || "",
        formatType: sticker.format_type || 1,
        url: stickerAssetUrl(sticker.id, sticker.format_type || 1, 160),
      }))
      .sort((left, right) => left.name.localeCompare(right.name)),
    permissions: {
      emojis: emojiResponse.ok,
      stickers: stickerResponse.ok,
    },
  });
}

function savedExpressionAccess(expression, context) {
  const botGuildIds = new Set((context.connection.guilds || []).map((guild) => guild.id));
  const confirmed = expression.sourceBotId === context.connection.botId
    && expression.sourceGuildId
    && botGuildIds.has(expression.sourceGuildId);
  return confirmed ? "available" : "discord-controlled";
}

function publicSavedExpression(expression, context) {
  const access = savedExpressionAccess(expression, context);
  return {
    id: expression.expressionId,
    type: expression.type,
    name: expression.name,
    animated: expression.animated === true,
    formatType: expression.formatType || null,
    url: expression.type === "emoji"
      ? `https://cdn.discordapp.com/emojis/${expression.expressionId}.${expression.animated ? "gif" : "png"}?size=64&quality=lossless`
      : stickerAssetUrl(expression.expressionId, expression.formatType || 1, 160),
    access,
    sourceGuildId: expression.sourceGuildId || null,
    savedAt: expression.createdAt,
    updatedAt: expression.updatedAt,
  };
}

function expressionFromMessage(message, type, expressionId) {
  if (type === "emoji") {
    const match = [...String(message.content || "").matchAll(/<(a?):([a-zA-Z0-9_]{1,32}):(\d{17,20})>/g)]
      .find((item) => item[3] === expressionId);
    return match ? { id: match[3], name: match[2], animated: Boolean(match[1]), formatType: null } : null;
  }
  const sticker = Array.isArray(message.sticker_items)
    ? message.sticker_items.find((item) => item.id === expressionId)
    : null;
  return sticker ? {
    id: sticker.id,
    name: sticker.name || "Discord sticker",
    animated: [2, 4].includes(sticker.format_type),
    formatType: sticker.format_type || 1,
  } : null;
}

async function getObservedDiscordMessage(context, channelId, messageId) {
  try {
    const result = await fetch(`${DISCORD_API}/channels/${channelId}/messages/${messageId}`, {
      headers: discordBotHeaders(context.token),
      signal: AbortSignal.timeout(8_000),
    });
    if (!result.ok) return { ok: false, status: result.status };
    return { ok: true, message: await result.json() };
  } catch {
    return { ok: false, status: 0 };
  }
}

async function saveObservedExpression(request, response) {
  let body;
  try {
    body = await readJsonBody(request);
  } catch {
    sendJson(response, 400, { code: "INVALID_REQUEST_BODY", error: "The expression request was invalid." });
    return;
  }
  const guildId = typeof body.guildId === "string" ? body.guildId.trim() : "";
  const channelId = typeof body.channelId === "string" ? body.channelId.trim() : "";
  const messageId = /^\d{17,20}$/.test(body.messageId || "") ? body.messageId : "";
  const expressionId = /^\d{17,20}$/.test(body.expressionId || "") ? body.expressionId : "";
  const type = ["emoji", "sticker"].includes(body.type) ? body.type : "";
  if (!messageId || !expressionId || !type) {
    sendJson(response, 400, { code: "EXPRESSION_INVALID", error: "Select a valid expression from a Discord message." });
    return;
  }
  ...
  const observed = await getObservedDiscordMessage(context, channelId, messageId);
  if (!observed.ok) {
    sendJson(response, observed.status === 403 || observed.status === 404 ? 403 : 502, {
      code: "EXPRESSION_SOURCE_UNAVAILABLE",
      error: observed.status === 403 || observed.status === 404
        ? "Discord no longer allows the bot to read that source message."
        : "Valax could not verify the source expression with Discord.",
    });
    return;
  }
  const expression = expressionFromMessage(observed.message, type, expressionId);
  if (!expression) {
    sendJson(response, 400, { code: "EXPRESSION_NOT_IN_MESSAGE", error: "That expression was not found in the selected Discord message." });
    return;
  }
  ...
  if (type === "emoji") {
    try {
      const guildEmojiResponse = await fetch(`${DISCORD_API}/guilds/${guildId}/emojis`, {
        headers: discordBotHeaders(context.token),
        signal: AbortSignal.timeout(8_000),
      });
      if (guildEmojiResponse.ok) {
        const guildEmojis = await guildEmojiResponse.json();
        if (guildEmojis.some((item) => item.id === expressionId)) sourceGuildId = guildId;
      }
    } catch {
      sourceGuildId = null;
    }
    ...
    try {
      const stickerResponse = await fetch(`${DISCORD_API}/stickers/${expressionId}`, {
        headers: discordBotHeaders(context.token),
        signal: AbortSignal.timeout(8_000),
      });
      if (stickerResponse.ok) {
        const sticker = await stickerResponse.json();
        sourceGuildId = sticker.guild_id || null;
        expression.name = sticker.name || expression.name;
        expression.formatType = sticker.format_type || expression.formatType;
        expression.animated = [2, 4].includes(expression.formatType);
      }
    } catch {
      sourceGuildId = null;
    }
  ...
  const collection = context.database.collection("savedExpressions");
  const filter = { userId: context.user._id, type, expressionId };
  const existing = await collection.findOne(filter);
  if (!existing && await collection.countDocuments({ userId: context.user._id }) >= MAX_SAVED_EXPRESSIONS) {
    sendJson(response, 409, { code: "EXPRESSION_LIBRARY_FULL", error: `Saved Expressions is limited to ${MAX_SAVED_EXPRESSIONS} items.` });
    return;
  }
  const now = new Date();
  await collection.updateOne(filter, {
    $set: {
      name: expression.name.slice(0, 64),
      animated: expression.animated,
      formatType: expression.formatType,
      sourceGuildId,
      sourceBotId: sourceGuildId ? context.connection.botId : null,
  ...
  sendJson(response, existing ? 200 : 201, {
    ...
    expression: publicSavedExpression(saved, context),
  });
}

async function expressionLibrary(request, response, url, method) {
  if (method === "POST") {
    await saveObservedExpression(request, response);
    return;
  }
  const guildId = url.searchParams.get("guildId")?.trim() || "";
  const context = await getServerContext(request, response, guildId, url.searchParams.get("botId") || "");
  if (!context) return;
  if (method === "GET") {
    const saved = await context.database.collection("savedExpressions")
      .find({ userId: context.user._id })
      .sort({ updatedAt: -1 })
      .limit(MAX_SAVED_EXPRESSIONS)
      .toArray();
    sendJson(response, 200, { expressions: saved.map((item) => publicSavedExpression(item, context)) });
    return;
  }
  ...
  const type = ["emoji", "sticker"].includes(body.type) ? body.type : "";
  const expressionId = /^\d{17,20}$/.test(body.expressionId || "") ? body.expressionId : "";
  if (!type || !expressionId) {
    sendJson(response, 400, { code: "EXPRESSION_INVALID", error: "Select a valid saved expression." });
    return;
  }
  await context.database.collection("savedExpressions").deleteOne({ userId: context.user._id, type, expressionId });
...
function allowedMentions(policy, userIds = [], repliedUser = false) {
  ...
  return { parse: [], replied_user: repliedUser };
}
...
async function sendServerMessage(request, response) {
  ...
  let attachment;
  try {
    ({ body, attachment } = await readMessageRequest(request));
  } catch (error) {
    const code = error?.code || (/too large/i.test(error?.message || "") ? "ATTACHMENT_TOO_LARGE" : "INVALID_REQUEST_BODY");
    sendJson(response, 400, {
      code,
      error: error?.code ? error.message : code === "ATTACHMENT_TOO_LARGE" ? "Attachments must be 4 MB or smaller." : "The message request was invalid.",
    });
    return;
  }
  ...
  const stickerIds = Array.isArray(body.stickerIds)
    ? [...new Set(body.stickerIds.map(String).filter((id) => /^\d{17,20}$/.test(id)))].slice(0, 3)
    : [];
  ...
  if ((!content && !attachment && stickerIds.length === 0) || content.length > 2_000) {
    sendJson(response, 400, {
      code: "MESSAGE_LENGTH_INVALID",
      error: "Add a message, attachment, or sticker. Message text can contain up to 2,000 characters.",
    });
    return;
  }
  ...
  let discordResponse;
  try {
    const messageRequest = await discordMessageBody({
    ...
    discordResponse = await fetch(`${DISCORD_API}/channels/${channelId}/messages`, {
      method: "POST",
      headers: { ...headers, ...messageRequest.headers },
      body: messageRequest.body,
      signal: AbortSignal.timeout(attachment ? 20_000 : 8_000),
    });
  } catch {
  ...
  if (discordResponse.status === 403 || discordResponse.status === 404) {
    const permissionHint = attachment
      ? " Verify the bot has Send Messages and Attach Files in this channel."
      : stickerIds.length
        ? " Verify the bot can send messages and use this server's stickers."
        : " Verify the bot has Send Messages in this channel.";
    sendJson(response, 403, { code: "CHANNEL_SEND_DENIED", error: `Discord denied permission to send in this channel.${permissionHint}` });
    return;
  }
  ...
  if (!discordResponse.ok) {
    const discordError = await discordResponse.json().catch(() => ({}));
    console.error(
      "[Valax message] Discord rejected a channel send:",
      discordResponse.status,
      discordError.code || "UNKNOWN",
      discordError.message || "No error message"
    );
    sendJson(response, 502, {
      code: "DISCORD_API_ERROR",
      discordCode: discordError.code || null,
      error: discordError.message
        ? `Discord could not send this message: ${discordError.message}`
        : "Discord could not send this message.",
    });
    return;
  }
  ...
  sendJson(response, 200, {
    ...
    published,
    mentionDelivery: {
      requested: [...new Set(mentionIds.filter((id) => /^\d{17,20}$/.test(id)))].length,
      resolved: Array.isArray(message.mentions) ? message.mentions.length : 0,
      notificationsEnabled: mentionPolicy !== "none",
      replyNotificationEnabled: Boolean(replyToId && body.notifyReplyAuthor === true),
    },
    createdAt,
  });
}
...
async function fetchDirectMessages(token, channelId, after = "") {
  const query = new URLSearchParams({ limit: after ? "100" : "50" });
  if (after) query.set("after", after);
  try {
    const messageResponse = await fetch(`${DISCORD_API}/channels/${channelId}/messages?${query}`, {
...
async function sendDirectMessageToMember(context, member, source, timeZone, { checkOptOut = true, attachment = null } = {}) {
  ...
  try {
    ...
    const discordResponse = await fetch(`${DISCORD_API}/channels/${opened.channel.id}/messages`, {
      method: "POST",
      headers: { ...discordBotHeaders(context.token), ...messageRequest.headers },
      body: messageRequest.body,
      signal: AbortSignal.timeout(attachment ? 20_000 : 8_000),
    });
    if (discordResponse.status === 429) {
...
async function touchDirectConversation(context, member, { opened = false, messaged = false } = {}) {
  const identity = publicGuildMember(member);
  const now = new Date();
  const timestamps = {
    ...(opened ? { lastOpenedAt: now } : {}),
    ...(messaged ? { lastMessageAt: now } : {}),
  };
  await context.database.collection("dmConversations").updateOne(
    {
      userId: context.user._id,
      botId: context.connection.botId,
...
async function getRecentDirectConversations(request, response, url) {
  const guildId = url.searchParams.get("guildId")?.trim() || "";
  const context = await getServerContext(request, response, guildId, url.searchParams.get("botId") || "");
  if (!context) return;
  const conversations = await context.database.collection("dmConversations")
    .find({ userId: context.user._id, botId: context.connection.botId, guildId })
    .sort({ updatedAt: -1 })
    .limit(16)
    .toArray();
  sendJson(response, 200, {
    conversations: conversations.map((conversation) => ({
      ...
      lastOpenedAt: conversation.lastOpenedAt || null,
...
async function getDirectConversation(request, response, url) {
  ...
  const after = /^\d{17,20}$/.test(url.searchParams.get("after") || "") ? url.searchParams.get("after") : "";
  const context = await getServerContext(request, response, guildId, url.searchParams.get("botId") || "");
  if (!context) return;
  ...
  const history = await fetchDirectMessages(context.token, opened.channel.id, after);
  if (!history.ok) {
  ...
  if (!after) await touchDirectConversation(context, member, { opened: true });
  sendJson(response, 200, {
    member: publicGuildMember(member),
    channelId: opened.channel.id,
    optedOut: after && history.messages.length === 0 ? null : directMessageOptedOut(history.messages, context.connection.botId),
...
async function sendDirectMessage(request, response) {
  ...
  try {
    ...
    sendJson(response, 400, {
      code,
      error: error?.code ? error.message : code === "ATTACHMENT_TOO_LARGE" ? "Attachments must be 4 MB or smaller." : "The direct message request was invalid.",
    });
    return;
  }
  const guildId = typeof body.guildId === "string" ? body.guildId.trim() : "";
  ...
  const timeZone = typeof body.timeZone === "string" && body.timeZone.length <= 64 ? body.timeZone : "UTC";
  const content = resolveDirectMessageTokens(source, context.guild.name, member, timeZone);
  if ((!content && !attachment) || content.length > 2_000) {
    sendJson(response, 400, { code: "MESSAGE_LENGTH_INVALID", error: "Add a message or attachment. Message text can contain up to 2,000 characters." });
    return;
  }
  ...
  const result = await sendDirectMessageToMember(context, member, source, timeZone, { attachment });
  if (!result.ok) {
  ...
  await touchDirectConversation(context, member, { opened: true, messaged: true });
...
export async function handleApiRequest(request, response) {
  ...
  try {
    ...
    if (method === "GET" && url.pathname === "/api/server/expressions") {
      await getServerExpressions(request, response, url);
      return true;
    }

    if (["GET", "POST", "DELETE"].includes(method) && url.pathname === "/api/server/expression-library") {
      await expressionLibrary(request, response, url, method);
      return true;
    }
    ...
    if (method === "GET" && url.pathname === "/api/server/dm-recent") {
      await getRecentDirectConversations(request, response, url);
      return true;
    }
